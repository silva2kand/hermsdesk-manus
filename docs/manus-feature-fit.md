...
| Wide Research | Wide Research workspace + Space Agent / agentic research prompts | Partial | Wide Research now creates stored runs, splits a brief into worker lanes/items, runs local-model workers in parallel, and synthesizes the findings. | Add live browser/search workers, source capture, dedupe, citations, and project-scoped research runs. |
