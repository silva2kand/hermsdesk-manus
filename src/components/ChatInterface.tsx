  ...
  const handleFrontDoorStatusIntent = async (outgoing: string) => {
    ...
    try {
      ...
      const content = [
        ...
        `Evidence kept for later: Z-reports ${(memory?.zReports?.length || 0).toLocaleString()} | Accounting/tax ${(memory?.accountingEvidence?.length || 0).toLocaleString()} | Legal/property ${(memory?.legalEvidence?.length || 0).toLocaleString()}.`,
